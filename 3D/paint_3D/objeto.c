...
void transladaObjeto(Objeto *objeto, float tx, float ty, float tz){

    float** mTranslada = criar_matriz();
    preenche_matriz_traslacao(mTranslada, tx, ty, tz);
    printf("%d", objeto->totalPontos);

    for (int i = 0; i < objeto->totalPontos; i++){
        modifica_pontos(mTranslada, &objeto->pontos[i].x, &objeto->pontos[i].y, &objeto->pontos[i].z, objeto->pontos[i].x, objeto->pontos[i].y, objeto->pontos[i].z);
    }
    calculaCentroide(objeto);
    desaloca_matriz(mTranslada);
}

void rotacionaObjeto(Objeto *objeto, float teta, int eixo){

    printf("Rotacionando");

    if(objeto->tipoObjeto == 0){ //Ele � um ponto
        float** mRotaciona = criar_matriz();
        preenche_matriz_rotacao(mRotaciona, teta);
        int i = 0;
        modifica_pontos(mRotaciona, &objeto->pontos[i].x, &objeto->pontos[i].y, &objeto->pontos[i].z, objeto->pontos[i].x, objeto->pontos[i].y, objeto->pontos[i].z);
        desaloca_matriz(mRotaciona);
    }
    else{
        float** mTranslada = criar_matriz();
        float** mRotaciona = criar_matriz();
        preenche_matriz_traslacao(mTranslada, objeto->x_meio, objeto->y_meio, objeto->z_meio);
        preenche_matriz_rotacao(mRotaciona, teta, eixo);
        ...
        for (int i = 0; i < objeto->totalPontos; i++){
            modifica_pontos(result2, &objeto->pontos[i].x, &objeto->pontos[i].y, &objeto->pontos[i].z, objeto->pontos[i].x, objeto->pontos[i].y, objeto->pontos[i].z);
        }
        //Desalocando
        desaloca_matriz(mRotaciona);
...
void escalaObjeto(Objeto *objeto, float sx, float sy, float sz){
    ...
    if(objeto->tipoObjeto != 0){
        ...
        preenche_matriz_traslacao(mTranslada, objeto->x_meio, objeto->y_meio, objeto->z_meio);
        preenche_matriz_escala(mEscala, sx, sy, sz);
...
int pickPoligono(Objeto *poligono, float x, float y, float t){
    ...
    for (int i = 0; i < poligono->totalPontos; i++){
        ...
        if((p1_y > y) && (p2_y > y)){
            ...
            //Arestas � direita, uma acima e outra abaixo do ponto em y
            ++intersect;
            ...
            // Se a intersec��o (xi) for maior que x, ent�o o tiro intercepta, caso contr�rio, n�o
            if (xi > x){
                ++intersect;
            }
